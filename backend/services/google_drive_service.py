...
SMALL_UPLOAD_CHUNK_SIZE_BYTES = 256 * 1024
        ...
        try:
            ...
            def upload_one(item: tuple[Path, str]) -> None:
                ...
                try:
                    ...
                    if "write operation timed out" in detail.lower():
                        cls._logger.warning(
                            "Resumable upload timed out; retrying with smaller chunk for %s",
                            local_file,
                        )
                        worker_service.files().create(
                            body={"name": local_file.name, "parents": [parent_id]},
                            media_body=MediaFileUpload(
                                ...
                                mimetype=mime_type,
                                resumable=True,
                                chunksize=SMALL_UPLOAD_CHUNK_SIZE_BYTES,
                            ),
                            fields="id",
                            supportsAllDrives=True,
